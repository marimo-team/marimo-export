...
const DEFAULT_MAX_JSON_VALUES = 100_000;
const MAX_JSON_DIAGNOSTIC_STRING = 256;
const MAX_JSON_NUMBER_LEXEME_BYTES = 1_024;

export function trimJsonWhitespace(bytes: Uint8Array): Uint8Array {
  let start = 0;
  let end = bytes.byteLength;
  while (start < end && isJsonWhitespace(bytes[start]!)) start += 1;
...
export function jsonValueLimit(input: number | undefined): number {
  if (input === undefined) return DEFAULT_MAX_JSON_VALUES;
