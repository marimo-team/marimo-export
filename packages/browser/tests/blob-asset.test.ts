...
import { defineBlobAssetLoader, openExport } from "../src/index.js";
import { hexBytes, exportFixture } from "./fixture.js";
...
test("decodes the exact native four-field msgspec envelope with direct metadata", async () => {
  ...
  const value = await notebookExport.state("alpha").output("view").load(loader);
...
test("rejects a non-native envelope field order", async () => {
  ...
  await expect(notebookExport.state("alpha").output("view").load(loader)).rejects.toMatchObject({
    code: "asset_invalid",
  });
});
