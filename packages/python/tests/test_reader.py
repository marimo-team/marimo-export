...
def _provenance(name: str, *, asset: bool = True) -> Provenance:
    return Provenance(
        cache_key=f"cell_cache/P_{name}.json",
...
def _publication(root: Path) -> tuple[Path, PublicationIndex, dict[str, bytes]]:
    npy = _npy((1, 2, 3))
    arrow = _arrow()
...
def test_reader_opens_resolves_and_reads_all_codec_payloads(tmp_path: Path) -> None:
    root, index, assets = _publication(tmp_path / "publication")

    publication = open_publication(root)
    baseline = publication.state("baseline")

    assert publication.path == root.absolute()
    assert publication.input_names == ("chart_width", "symbols_selector")
    assert publication.output_names == ("count", "array", "table", "chart")
    assert publication.notebook == index.notebook
    assert publication.producer == index.producer
    assert tuple(state.name for state in publication.states()) == ("baseline", "msft")
...
def test_state_resolution_is_exact_and_immutable(tmp_path: Path) -> None:
    ...
    assert baseline.resolve({"symbols_selector": ["MSFT"]}) is publication.state("msft")
    assert publication.resolve(
        {"chart_width": 800, "symbols_selector": ["MSFT"]}
    ) is publication.state("msft")
    ...
    with pytest.raises(PublicationError) as raised:
        publication.resolve({"chart_width": 480, "symbols_selector": ["MSFT"]})
...
def test_named_lookup_errors_are_typed(tmp_path: Path) -> None:
    ...
    with pytest.raises(PublicationError) as raised:
        publication.state("baseline").output("missing")
    assert raised.value.code == "output_not_found"


def test_codec_specific_read_methods_reject_other_codecs(tmp_path: Path) -> None:
    root, _, _ = _publication(tmp_path / "publication")
    state = open_publication(root).state("baseline")

    with pytest.raises(PublicationError, match="no asset"):
        state.output("count").asset_bytes()
    with pytest.raises(PublicationError, match="scalar codec"):
        state.output("array").scalar()
    with pytest.raises(PublicationError, match="BlobAsset"):
...
def test_open_reads_only_the_index(tmp_path: Path) -> None:
    ...
    assert publication.state("baseline").output("count").scalar() == 3
    with pytest.raises(IntegrityError):
        publication.state("baseline").output("array").asset_bytes()
...
def test_asset_digest_is_verified_before_codec_decode(tmp_path: Path) -> None:
    ...
    with pytest.raises(IntegrityError, match="SHA-256"):
        open_publication(root).state("baseline").output("array").asset_bytes()
...
def test_blob_envelope_must_match_its_descriptor(tmp_path: Path) -> None:
    ...
    changed_index = PublicationIndex.from_value(wire)
    old_path = path
    new_descriptor = cast(
        BlobAssetDescriptor,
    ...
    with pytest.raises(IntegrityError, match="media type"):
        open_publication(root).state("baseline").output("chart").blob_asset()
...
def test_reader_rejects_symlinked_roots_and_assets(tmp_path: Path) -> None:
    ...
    with pytest.raises(PublicationError, match="real directory"):
        open_publication(linked_root)
...
def test_special_scalars_round_trip_through_reader(tmp_path: Path) -> None:
    ...
    for name, value in values.items():
        ...
        root.mkdir()
        (root / "index.json").write_bytes(index.to_bytes())
        loaded = open_publication(root).state("state").output("value").scalar()
        if name == "nan":
