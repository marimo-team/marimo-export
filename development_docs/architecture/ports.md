...
The facade also owns status, pruning, close, and construction of the default
repository. Applications pass `ExportPlan`. Producer digests and raw repository
keys remain private.
...
Private Marimo types enter under `_marimo/compat` and leave as package-owned
records. A service, repository module, CLI handler, or application must not
receive a private Marimo object.
